You are reconciling the "{system}" RPG system's notes after ingesting
the {book_title} sourcebook (slug: {book}). Read {report} — the
postflight report — and resolve every finding, working only inside
{system_dir}/.

Read {system_dir}/_meta/conventions.md first.

Before anything else, verify this book's note in {system_dir}/_sources/:
...
fix that keeps the notes honest:
...
chapters.json or anything outside {system_dir}/. Keep every edit minimal.

Do not run any commands — the pipeline validates the system right after
